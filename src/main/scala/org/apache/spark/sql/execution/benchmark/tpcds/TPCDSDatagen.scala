 ...
 */
class TPCDSTables(sqlContext: SQLContext, scaleFactor: Int) extends Serializable {
  ...
  private object Table {
    ...
    def apply(name: String, partitionColumns: Seq[String], sortColumn: String, schemaString: String): Table = {
      new Table(name, partitionColumns, sortColumn, StructType.fromDDL(schemaString))
    }
  }

  private case class Table(name: String, partitionColumns: Seq[String], sortColumn: String, schema: StructType) {
    def nonPartitioned: Table = {
      Table(name, Nil, sortColumn, schema)
    }
    ...
    def useDoubleForDecimal(): Table = {
      ...
      Table(name, partitionColumns, sortColumn, StructType(newFields))
    }
    ...
    def genData(
      ...
      if (lsmTree) {
        var maxLevelBytes = maxBytesForLevelBase
        ...
        while (false) {
          ...
          val rowCount = maxLevelBytes / 8
          // Get rows in current level
          val levelData = data.limit(rowCount)
          // A copy of level data
          // Get and drop rows for each SSTable from this
          var tempLevelData = levelData
          val levelLocation = s"$location/level-$level"

          var fileID = 0
          var fileNum = tempLevelData.count() * 8 / maxFileSize
          if (fileNum == 0) {
            fileNum = 1
          } 

          tempLevelData.write.bucketBy(fileNum.toInt, sortColumn)
            .sortBy(sortColumn)
            .format(format)
            .mode(mode)
            .option("path", levelLocation)
            .saveAsTable(s"${name}_${level}")

          level += 1
          // Drop rows that have been saved in current level
          maxLevelBytes = maxLevelBytes * maxBytesForLevelMultiplier
          data = data.except(levelData).orderBy(sortColumn)
        }
      } else {
  ...
  private val tables = Seq(
    ...
    Table("catalog_returns",
      ...
      sortColumn = "cr_returned_date_sk",
      """
    ...
    Table("store_returns",
      ...
      sortColumn = "sr_returned_date_sk",
      """
    ...
    Table("web_returns",
      ...
      sortColumn = "wr_returned_date_sk",
      """
...
object TPCDSDatagen {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tpcdsTables = new TPCDSTables(spark.sqlContext, datagenArgs.scaleFactor.toInt)
    tpcdsTables.genData(
